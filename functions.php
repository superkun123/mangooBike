...
//	    wp_enqueue_script('jquery','//ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js','','',true);
		...
		wp_enqueue_script('bootstrap', '//stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js','','',true);
		wp_enqueue_script('wow', get_template_directory_uri().'/js/wow.min.js','','',true);
		wp_enqueue_script('main', get_template_directory_uri().'/js/main.js','','',true);

		wp_enqueue_script('TweenMax', 'http://cdnjs.cloudflare.com/ajax/libs/gsap/2.0.2/TweenMax.min.js','','',true);
		wp_enqueue_script('TimelineMax', 'http://cdnjs.cloudflare.com/ajax/libs/gsap/2.0.2/TimelineMax.min.js','','',true);
		wp_enqueue_script('ScrollMagic', 'http://cdnjs.cloudflare.com/ajax/libs/ScrollMagic/2.0.6/ScrollMagic.min.js','','',true);
		wp_enqueue_script('animation', 'http://cdnjs.cloudflare.com/ajax/libs/ScrollMagic/2.0.6/plugins/animation.gsap.min.js','','',true);

		wp_enqueue_script('ttt', get_template_directory_uri().'/js/final.js','','',true);
	}
}
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
	    if(is_admin()) return false; // если мы в админке - ничего не делаем
		wp_enqueue_style( 'bs', 'http://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css' ); // бутстрап
